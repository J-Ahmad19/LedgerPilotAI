...
function StatCard({ label, value, trend, accent, delay }: StatCardProps) {
  ...
  return (
    ...
    >
      ...
      <div ref={valueRef} className={`text-2xl font-bold ${colors.text}`}>
        {displayValue}
      </div>
      <div className={`text-xs font-semibold mt-1 ${colors.text} opacity-80`}>{trend}</div>
    </motion.div>
  );
}
...
export default function DashboardMockup() {
  ...
  return (
    <div className="relative w-full max-w-4xl mx-auto group">
      {/* Outer Glow */}
      <div className="absolute -inset-6 bg-gradient-to-r from-ledger-blue/15 via-ledger-emerald/10 to-ledger-violet/15 rounded-[2rem] blur-3xl opacity-40 group-hover:opacity-60 transition-opacity duration-700" />

      {/* Main Dashboard */}
      <div className="relative glass-card border-white/[0.08] rounded-2xl shadow-2xl shadow-black/50 overflow-hidden">
        {/* Browser Bar */}
        <div className="flex items-center gap-3 px-5 py-3.5 border-b border-white/[0.05] bg-white/[0.02]">
          ...
          <div className="flex-1 flex justify-center">
            <div className="flex items-center gap-2 px-4 py-1.5 rounded-lg bg-white/[0.04] border border-white/[0.06]">
              ...
              <span className="text-xs text-slate-500 font-mono">app.ledgerpilot.ai/dashboard</span>
            </div>
          </div>
        </div>
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm font-medium text-slate-300">Reconciliation Progress</span>
              </div>
            ...
            <div className="flex justify-between mt-3">
              <span className="text-xs text-slate-500">12,615 matched</span>
              <span className="text-xs text-slate-500">232 pending</span>
            </div>
          </motion.div>
          ...
          >
            <div className="flex items-center justify-between mb-5">
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm font-medium text-slate-300">Cash Position Variance</span>
              </div>
              <span className="text-xs px-3 py-1.5 bg-emerald-500/10 text-emerald-400 rounded-lg font-semibold border border-emerald-500/20">
                Real-time
              </span>
            </div>
      ...
      <motion.div
        ...
        className="absolute -left-6 lg:-left-12 top-[30%] glass-card p-3.5 shadow-xl shadow-black/20 hidden lg:block animate-float"
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-[10px] text-slate-500 uppercase tracking-wider font-medium">Auto-Matched</div>
      ...
      <motion.div
        ...
        className="absolute -right-6 lg:-right-12 bottom-[25%] glass-card p-3.5 shadow-xl shadow-black/20 hidden lg:block animate-float-delayed"
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-[10px] text-slate-500 uppercase tracking-wider font-medium">AI Resolving</div>
            <div className="text-sm font-bold text-white">3 in queue</div>
          </div>
        </div>
      </motion.div>
      ...
      <motion.div
        ...
        className="absolute left-1/2 -translate-x-1/2 -bottom-6 glass-card px-4 py-2.5 shadow-xl shadow-black/20 hidden md:block"
      >
        <div className="flex items-center gap-2">
          <div className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
          <span className="text-xs text-slate-400">Cash sync</span>
