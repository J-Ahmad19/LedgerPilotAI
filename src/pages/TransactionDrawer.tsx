        ...
        <div className="space-y-8 pb-12">
          ...
          <section className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="charm-panel p-4 bg-charm-section">
                <div className="text-xs font-semibold text-charm-heading uppercase tracking-wider mb-2">Normalized</div>
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    <span className="text-charm-muted">Reference</span>
              ...
              <div className="space-y-4">
                {data.matches?.map((m: any) => (
                  <div key={m.id} className="charm-panel overflow-hidden">
                    <div className="bg-charm-section p-3 border-b border-charm-border flex items-center justify-between text-sm">
                      <span className="font-semibold text-charm-heading uppercase tracking-wider text-xs">{m.matchType} Match</span>
                      <span className="font-mono px-2 py-0.5 rounded text-xs font-medium bg-white shadow-sm border border-charm-border text-charm-heading">Score: {(Number(m.confidenceScore) * 100).toFixed(1)}%</span>
                    </div>
                    <div className="p-4 space-y-4">
                      ...
                      {m.matchType === 'AI' && (
                        <div className="border-t border-charm-border -mx-4 -mb-4 p-4 mt-2 bg-charm-base">
                          <div className="flex justify-between items-center mb-2">
                            <span className="text-xs font-semibold text-charm-muted uppercase">AI Reasoning</span>
                            {m.aiDecision && (
                          ...
                          <p className="text-sm text-charm-body mb-2">{m.reason}</p>
                          {m.aiEvidence && (
                            <div className="text-xs text-charm-muted bg-charm-surface p-2 rounded border border-charm-border font-mono overflow-x-auto whitespace-pre-wrap">
          ...
          <section className="space-y-4">
            ...
            {data.auditHistory?.length === 0 ? (
              ...
              <div className="relative border-l border-charm-border ml-3 space-y-6 mt-4">
                {data.auditHistory?.map((log: any) => (
                  <div key={log.id} className="relative pl-6">
                    <div className="absolute w-2.5 h-2.5 bg-charm-border rounded-full -left-[5px] top-1.5 shadow-[0_0_0_4px_var(--charm-surface)]"></div>
